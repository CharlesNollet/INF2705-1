...
void chargerNuanceurs() {
    ...
    std::cout << "Charger le nuanceur de base" << std::endl;
    {
        ...
        // if ( ( locVertexBase = glGetAttribLocation( progBase, "Vertex" ) ) == -1 ) std::cerr <<
        // "!!! pas trouvé la \"Location\" de Vertex" << std::endl;
    ...
    std::cout << "Charger le nuanceur de rétroaction" << std::endl;
    {
        ...
        if((loctempsRestantRetroaction = glGetAttribLocation(progRetroaction, "tempsRestant")) ==
           -1)
            std::cerr << "!!! pas trouvé la \"Location\" de tempsRestant" << std::endl;
        ...
        if((locpositionPuitsRetroaction = glGetUniformLocation(progRetroaction, "positionPuits")) ==
           -1)
...
void initialiser() {
    ...
    glBufferData(GL_ARRAY_BUFFER,
                 sizeof(part),
                 NULL,
                 GL_STREAM_DRAW); // on ne donne rien sinon la taille
    ...
    glVertexAttribPointer(locVertex,
                          3,
                          GL_FLOAT,
                          GL_FALSE,
                          sizeof(Part),
    ...
    glVertexAttribPointer(locColor,
                          4,
                          GL_FLOAT,
                          GL_FALSE,
                          sizeof(Part),
    ...
    glVertexAttribPointer(loctempsRestant,
                          1,
                          GL_FLOAT,
                          GL_FALSE,
                          sizeof(Part),
    ...
    glVertexAttribPointer(locvitesse,
                          3,
                          GL_FLOAT,
                          GL_FALSE,
                          sizeof(Part),
...
void FenetreTP::afficherScene() {
    ...
    glVertexAttribPointer(loctempsRestant,
                          ...
                          reinterpret_cast<void*>(offsetof(Part, tempsRestant)));
    // À MODIFIER (partie 3)
    // glVertexAttribPointer( locvitesse, 3, GL_FLOAT, GL_FALSE, sizeof(Part),
    // reinterpret_cast<void*>( offsetof(Part,vitesse) ) );
    glVertexAttribPointer(locColor,
                          4,
                          GL_FLOAT,
...
void FenetreTP::clavier(TP_touche touche) {
    // quelques variables pour n'imprimer qu'une seule fois la liste des touches lorsqu'une touche
    // est invalide
    bool toucheValide = true; // on suppose que la touche est connue
    static bool listerTouchesAFAIRE =
            true; // si la touche est invalide, on imprimera la liste des touches
    ...
    switch(touche) {
        ...
        case TP_CROCHETDROIT: {
            ...
            nparticules *= 1.2;
            if(nparticules > MAXNPARTICULES)
                nparticules = MAXNPARTICULES;
            ...
            for(unsigned int i = nparticulesPrec; i < nparticules; ++i)
                ptr[i].tempsRestant =
                        0.0; // la particule sera initialisée par le nuanceur de rétroaction
    ...
    {
        listerTouchesAFAIRE =
                true; // ... on imprimera la liste des touches à la prochaine touche invalide
