...
// le programme de nuanceurs de base
GLuint progBase;
...
struct LightModelParameters {
	...
	int localViewer;   // doit-on prendre en compte la position de l'observateur? (local ou à
	                   // l'infini)
...
bool afficheNormales =
        false; // indique si on utilise les normales comme couleurs (utile pour le débogage)
...
struct VueStereo {
	...
	GLdouble zecran;   // la position du plan de l'écran: les objets affichés en avant de ce plan
	                   // «sortiront» de l'écran
...
void calculerPhysique() {
	if(enmouvement) {
		...
		if(facteurDeform < 0.1)
			sensZ = +1.0;
		else if(facteurDeform > 1.0)
			sensZ = -1.0;
#endif
...
void charger1Texture(std::string fichier, GLuint& texture) {
	...
	if((pixels = ChargerImage(fichier, largeur, hauteur)) != NULL) {
		...
		glTexImage2D(
		        GL_TEXTURE_2D, 0, GL_RGBA, largeur, hauteur, 0, GL_RGBA, GL_UNSIGNED_BYTE, pixels);
...
void chargerNuanceurs() {
...

	{
		...
		std::ostringstream
		        preambule; // ce préambule sera ajouté avant le contenu du fichier du nuanceur
		...
		// attacher le nuanceur de sommets
		const GLchar* chainesSommets[2] = {preambulechar,
		...
		const GLchar* chainesTessCtrl[2] = {preambulechar,
		                                    ProgNuanceur::lireNuanceur("nuanceurTessCtrl.glsl")};
		...
		const GLchar* chainesTessEval[2] = {preambulechar,
		                                    ProgNuanceur::lireNuanceur("nuanceurTessEval.glsl")};
		...
		const GLchar* chainesGeometrie[2] = {preambulechar,
		                                     ProgNuanceur::lireNuanceur("nuanceurGeometrie.glsl")};
		...
		// attacher le nuanceur de fragments
		const GLchar* chainesFragments[2] = {preambulechar,
		                                     ProgNuanceur::lireNuanceur("nuanceurFragments.glsl")};
		...
		if((locmatrProj = glGetUniformLocation(prog, "matrProj")) == -1)
			std::cerr << "!!! pas trouvé la \"Location\" de matrProj" << std::endl;
		if((locmatrNormale = glGetUniformLocation(prog, "matrNormale")) == -1)
			std::cerr << "!!! pas trouvé la \"Location\" de matrNormale" << std::endl;
		// if ( ( loctextureDepl = glGetUniformLocation( prog, "textureDepl" ) ) == -1 ) std::cerr
		// << "!!! pas trouvé la \"Location\" de textureDepl" << std::endl; if ( ( loctextureCoul =
		// glGetUniformLocation( prog, "textureCoul" ) ) == -1 ) std::cerr << "!!! pas trouvé la
		// \"Location\" de textureCoul" << std::endl;
		loctextureDepl = glGetUniformLocation(prog, "textureDepl");
		loctextureCoul = glGetUniformLocation(prog, "textureCoul");
		if((locfacteurDeform = glGetUniformLocation(prog, "facteurDeform")) == -1)
			std::cerr << "!!! pas trouvé la \"Location\" de facteurDeform" << std::endl;
		if((locbDim = glGetUniformLocation(prog, "bDim")) == -1)
			std::cerr << "!!! pas trouvé la \"Location\" de bDim" << std::endl;
		if((locTessLevelInner = glGetUniformLocation(prog, "TessLevelInner")) == -1)
			std::cerr << "!!! pas trouvé la \"Location\" de TessLevelInner" << std::endl;
		if((locTessLevelOuter = glGetUniformLocation(prog, "TessLevelOuter")) == -1)
			std::cerr << "!!! pas trouvé la \"Location\" de TessLevelOuter" << std::endl;
		if((indLightSource = glGetUniformBlockIndex(prog, "LightSourceParameters")) ==
		   GL_INVALID_INDEX)
			std::cerr << "!!! pas trouvé l'\"index\" de LightSource" << std::endl;
		if((indFrontMaterial = glGetUniformBlockIndex(prog, "MaterialParameters")) ==
		   GL_INVALID_INDEX)
			std::cerr << "!!! pas trouvé l'\"index\" de FrontMaterial" << std::endl;
		if((indLightModel = glGetUniformBlockIndex(prog, "LightModelParameters")) ==
		   GL_INVALID_INDEX)
...
void initialiser() {
	...
	{
		// les coordonnées
		const GLfloat sommets[] = {0.0, 1.0, 1.0, 1.0, 1.0, 0.0, 0.0, 0.0};
...
void afficherDecoration() {
	...
	for(int i = 0; i < 2; ++i) {
		...
		if(!positionnelle) {
			...
			{
				glm::mat4 a;
				a[0] = glm::vec4(2 * LightSource.position[i].x,
				                 2 * LightSource.position[i].y,
				                 2 * LightSource.position[i].z,
				                 0.0);
				a[1] = glm::vec4(2 * LightSource.position[i].x,
				                 -2 * LightSource.position[i].y,
				                 2 * LightSource.position[i].z,
				                 0.0);
				a[2] = glm::vec4(2 * LightSource.position[i].x,
				                 2 * LightSource.position[i].y,
				                 -2 * LightSource.position[i].z,
				                 0.0);
		...
		matrModel.PushMatrix();
		{
			matrModel.Translate(LightSource.position[i].x,
			                    LightSource.position[i].y,
			                    LightSource.position[i].z);
			glUniformMatrix4fv(locmatrModelBase, 1, GL_FALSE, matrModel);
			sphereLumi->afficher();
		}
		matrModel.PopMatrix();
...
void FenetreTP::afficherScene() {
	...
	// glUniformMatrix4fv( locmatrProj, 1, GL_FALSE, matrProj ); // inutile car on modifie ensuite
	// la projection
	glUniformMatrix4fv(locmatrVisu, 1, GL_FALSE, matrVisu);
	glUniformMatrix4fv(locmatrModel, 1, GL_FALSE, matrModel);
	// (partie 1: ne pas oublier de calculer et donner une matrice pour les transformations des
	// normales)
	glUniformMatrix3fv(
	        locmatrNormale,
	        1,
...
void FenetreTP::clavier(TP_touche touche) {
	switch(touche) {
		...
		case TP_i: // Augmenter le niveau de tessellation interne
			++TessLevelInner;
			std::cout << " TessLevelInner=" << TessLevelInner
			          << " TessLevelOuter=" << TessLevelOuter << std::endl;
		...
		case TP_0: // Revenir à la surface de base (ne pas utiliser de textures)
			...
			std::cout << " indiceFonction=" << indiceFonction << " indiceTexture=" << indiceTexture
			          << " indiceDiffuse=" << indiceDiffuse << std::endl;
		...
		case TP_c: // Varier l'indice de la texture utilisée pour la composante diffuse de la
		           // couleur
			if(++indiceDiffuse > 9)
				indiceDiffuse = 0;
			std::cout << " indiceFonction=" << indiceFonction << " indiceTexture=" << indiceTexture
	...
	if(pressed) {
		...
		switch(button) {
			...
			case TP_BOUTON_MILIEU: // Déplacer la lumière en profondeur uniquement
				...
				break;
			case TP_BOUTON_DROIT: // Déplacer la lumière à la position de la souris (sans changer la
			                      // profondeur)
				deplace = deplaceLum;
				break;
		}
		if(deplace != deplaceCam) {
			...
			glm::vec2 ecranLumi0 =
			        glm::vec2(glm::project(LightSource.position[0].xyz(), VM, P, cloture));
			glm::vec2 ecranLumi1 =
			        glm::vec2(glm::project(LightSource.position[1].xyz(), VM, P, cloture));
			glm::vec2 ecranXY(x, hauteur_ - y);
			curLumi = (glm::distance(ecranLumi0, ecranXY) < glm::distance(ecranLumi1, ecranXY)) ? 0
			                                                                                    : 1;
		}
	} else {
	...
	if(pressed) {
		...
		switch(deplace) {
			...
			case deplaceLumProfondeur:
				...
				LightSource.position[curLumi] = glm::vec4(glm::unProject(ecranLumi, VM, P, cloture),
				                                          positionnelle ? 1.0 : 0.0);
				break;
			case deplaceLum:
				// placer la lumière à la nouvelle position (en utilisant la profondeur actuelle)
				glm::vec3 ecranPos(x, hauteur_ - y, ecranLumi[2]);
				LightSource.position[curLumi] = glm::vec4(glm::unProject(ecranPos, VM, P, cloture),
				                                          positionnelle ? 1.0 : 0.0);
				break;
		}
